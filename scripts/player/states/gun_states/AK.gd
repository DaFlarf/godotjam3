...
@onready var AK_cancel_timer = $AKCancelTimer
@onready var AK_finished_timer = $AKFinishedTimer
...
func _on_ak_cancel_timer_timeout() -> void:
	parent.spawn_AK_boolet()
	sfx.play()
	cancellable = true
	AK_finished_timer.start()


func _on_ak_finished_timer_timeout() -> void:
	finished = true
